...
export default function AdminDashboard() {
  ...
  const stats = [
    ...
    { label: "Gallery Assets", count: 42, icon: <ImageIcon className="w-5 h-5 text-purple-500" /> },
    { label: "Active Downloads", count: 8, icon: <Download className="w-5 h-5 text-emerald-500" /> },
  ...
  return (
    <div className="pt-20 bg-bg-gray min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        ...
        {/* Admin Header */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-gray-200 pb-6 text-left">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              className="bg-white border border-gray-200 text-xs font-semibold px-3 py-1.5 rounded-xl focus:outline-none focus:border-primary-navy"
              value={selectedRole}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* Sidebar Navigation */}
          <div className="lg:col-span-3 bg-white rounded-3xl p-6 border border-gray-150 shadow-sm space-y-6 h-fit text-left">
            <div className="space-y-1.5">
              ...
              <ul className="space-y-1 text-xs">
                <li>
                  <button className="w-full flex items-center gap-3 px-3 py-2 rounded-xl bg-primary-navy text-white font-bold">
                    <LayoutDashboard className="w-4 h-4" /> Dashboard Home
                  </button>
                </li>
                <li>
                  <button className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-muted-text hover:bg-bg-gray hover:text-primary-navy font-semibold transition-colors">
          ...
          <div className="lg:col-span-9 space-y-8 text-left">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {stats.map((stat, idx) => (
                <div key={idx} className="bg-white rounded-3xl p-6 border border-gray-150 shadow-sm flex items-center justify-between">
                  <div className="space-y-1">
                    <span className="text-[10px] uppercase font-bold text-muted-text">{stat.label}</span>
                    <h3 className="font-heading font-black text-2xl text-primary-navy">{stat.count}</h3>
                  </div>
                  <div className="w-10 h-10 rounded-xl bg-bg-gray flex items-center justify-center">
                    {stat.icon}
                  </div>
                </div>
              ))}
            ...
            <div className="bg-white rounded-3xl border border-gray-150 shadow-sm overflow-hidden">
              <div className="p-6 border-b border-gray-100 flex items-center justify-between flex-wrap gap-4">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 bg-primary-navy hover:bg-emergency-red text-white font-bold px-4 py-2 rounded-xl transition-colors text-xs shadow-sm"
                >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-bg-gray text-primary-navy font-bold border-b border-gray-100 text-left">
                  ...
                  <tbody className="divide-y divide-gray-100 text-muted-text">
                    {recentContent.map((item, idx) => (
                      <tr key={idx} className="hover:bg-bg-gray/40 transition-colors">
